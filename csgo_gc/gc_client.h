...
void ClientGC::HandleSOCacheRequest()
{
    ...
    // Добавляем PersonaDataPublic с Prime статусом
    CMsgSOCacheSubscribed_SubscribedType *personaType = message.add_objects();
    personaType->set_type_id(SOTypePersonaDataPublic);

    CSOPersonaDataPublic personaData;
    personaData.set_player_level(GetConfig().Level());
    personaData.set_elevated_state(ElevatedStatePrime);
    personaData.mutable_commendation()->set_cmd_friendly(GetConfig().CommendedFriendly());
    personaData.mutable_commendation()->set_cmd_teaching(GetConfig().CommendedTeaching());
    personaData.mutable_commendation()->set_cmd_leader(GetConfig().CommendedLeader());

    CMsgSOSingleObject *personaObj = personaType->add_object_data();
    personaObj->set_type_id(SOTypePersonaDataPublic);
    personaObj->set_object_data(personaData.SerializeAsString());
